...
| --- | --- | --- | --- | --- | --- |
| Discover Claude plugins, Copilot extensions, Skills, and MCP servers from one place | `src/lib/validation/contracts.ts` (`CatalogKindSchema`), `config/registries.json` (live plugin sources + fallback registries), `src/catalog/sync.ts` (kind normalization/count logging) | `tests/integration/functionality-claims.spec.ts` (`discovers all four ecosystems and supports filtering by kind`), `tests/unit/registry-config.spec.ts`, `tests/integration/cli-flow.spec.ts` (`filters recommendations by requested kinds`) | `catalog-sync.yml` publishes per-kind item counts in summary | pass | Removed dead plugin APIs and added active sources (`claude.com/connectors`, GitHub marketplace manifests) while preserving fallback entries. |
